...
void main()
{
    ...
    // Pattern 15
    // float barX = step(0.4, mod(vUv.x * 10.0, 1.0)) * step(0.8, mod(vUv.y * 10.0+.2, 1.0));
    // float barY = step(0.8, mod(vUv.x * 10.0+.2, 1.0)) * step(0.4, mod(vUv.y * 10.0, 1.0));
    // float strength = barX + barY;
    ...
    // strength *= step(.2, max(abs(vUv.x-.5),abs(vUv.y-.5)));
    ...
    //  Pattern 33
    // float strength = step(.25,distance(vUv,vec2(0.5))); //  distance for circle

    //  Pattern 34
    // float strength = abs(    distance(vUv,vec2(0.5))-.3);

    //  Pattern 35
    // float strength = step(0.01,abs(distance(vUv,vec2(0.5))-.3))  ;

    //  Pattern 36
    // float strength = 1.0- step(0.01,abs(distance(vUv,vec2(0.5))-.3));
    ...
    // );
    // float strength = 1.0-step(0.01,abs(distance(waveUv,vec2(0.5))-.3));
    ...
    //     vUv.y+sin(vUv.x*30.0)*.1
    // );
    // float strength = 1.0-step(0.01,abs(distance(waveUv,vec2(0.5))-.25));
    ...
    // float strength = angle/(PI*2.0) + 0.5 ;
      ...
      //  Pattern 43
    // float angle = atan(vUv.x-.5,vUv.y-.5);
    // float strength = mod(angle*4.0,1.0);
    ...
    // float strength = 1.0-step(.1,mod(cnoise(vUv*10.0),.3));
    // float strength = sin(cnoise(vUv*10.0)*25.0);

    //  Pattern 50 (Perlin Noise)
    float strength =step(.9,sin(cnoise(vUv*10.0)*10.0));
    // float strength = step(.1,sin(cnoise(vUv*10.0)*25.0));

    //  Clamp the strength
    strength = clamp(strength,0.0,1.0); // limit the strength between 0-1
